...
class Api_pcs extends REST_Controller {
    ...
    private $secret_key = "password_sungguh_rahasia";
    ...
    public function login_post(){
        ...
        if (empty($result)) {
            ...
            $payload["exp"] = $date->getTimestamp() + 3600 * 24 * 10; // 10 Hari token expire
            $data_json = array(
                "success" => true,
    ...
    public function produk_put(){
        ...
        if ($this->put("id")!="" && !$this->M_produk->cekProdukExist($this->put("id"))) {
            array_push($validation_message, "ID tidak ditemukan");
        }
    ...
    public function item_transaksi_post(){
        ...
        if ($this->input->post("produk_id")!="" && !$this->M_produk->cekProdukExist($this->input->post("produk_id"))) {
            array_push($validation_message, "Produk Id Id tidak ditemukan");
        }
